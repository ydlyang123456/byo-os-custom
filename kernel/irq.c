...
static void pic_remap(void) {
    ...
    outb(0x21, 0xFB); /* mask all except IRQ1 (keyboard) */
    outb(0xA1, 0xFF); /* mask all slave IRQs */
}

/* Forward declare external IDT setup function */
extern void idt_set_gate64(uint8_t num, uint64_t base, uint16_t sel, uint8_t flags);

void irq_init(void) {
    pic_remap();

    /* Register IRQ stubs in IDT (vectors 32-47) */
    void *irq_stubs[16] = { irq32, irq33, irq34, irq35, irq36, irq37, irq38, irq39,
                            irq40, irq41, irq42, irq43, irq44, irq45, irq46, irq47 };
    for (int i = 0; i < 16; i++) {
        uint64_t addr = (uint64_t)irq_stubs[i];
        idt_set_gate64(32 + i, addr, 0x08, 0x8E);
    }

    /* Register handler function pointers */
