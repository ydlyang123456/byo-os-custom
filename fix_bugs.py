...
# Fix 1: cmd_find - extract just filename from "[DIR]  name" or "[FILE] name"
old_find = '''static void cmd_find(int argc, char args[][CMD_MAX_LEN]) {
...
new_find = '''static void cmd_find(int argc, char args[][CMD_MAX_LEN]) {
    if (argc < 2) { vga_puts("Usage: find <pattern>\\n"); return; }
    memset(file_buf, 0, FILE_BUF_SIZE);
    fs_list_dir(".", file_buf, FILE_BUF_SIZE);
    char *p = file_buf;
    while (*p) {
        ...
        /* Extract just the filename, skip [DIR] or [FILE] prefix */
        char *name = line;
...
# Fix 2: cmd_du - show per-file sizes instead of filesystem stats
...
# Fix 3: cmd_hexdump - fix the itoa hex output (should be zero-padded)
# The current implementation looks correct, but let's make sure itoa works for hex
